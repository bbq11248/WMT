...
SELECT 201905151321 - 201906031317 FROM DUAL;


DELETE FROM MOVIE_PLAY 
	WHERE (SELECT TO_NUMBER(REPLACE(REPLACE(MOVIE_START_TIME, ' ', ''), ':', '')) FROM MOVIE_PLAY WHERE MOVIE_PLAY_NO = 'MPN1') - (SELECT TO_NUMBER(REPLACE(TO_CHAR(SYSDATE, 'YYYYMMDD HH24MI'), ' ', '')) FROM DUAL) <= 0;

SELECT REPLACE(TO_CHAR(SYSDATE, 'YYYYMMDD HH24MI'), ' ', '') FROM DUAL;


SELECT SEAT_NO FROM TICKETING JOIN MOVIE_PLAY USING(MOVIE_PLAY_NO) WHERE MOVIE_START_TIME = '20190515 17:21';

SELECT MOVIE_START_TIME, COUNT(MOVIE_START_TIME) FROM TICKETING JOIN MOVIE_PLAY USING(MOVIE_PLAY_NO)
GROUP BY MOVIE_START_TIME
HAVING COUNT(MOVIE_START_TIME)>2;
