...
VALUES(MOVIEMEMBER_SEQ.NEXTVAL, 'admin', 'admin', '관리자', '관리자님', 190612, 01012341234, '강남구 역삼동', 'wlstnr7833@naver.com', 'A', SYSDATE);


INSERT INTO MILEAGEMANAGER
(MILEAGE_SEQ, ID, MILEAGE)
VALUES('MG'||MILEAGE_SEQ.NEXTVAL, 'admin', 0);
...
UPDATE MOVIE_PLAY
			SET MOVIE_NO = (SELECT MOVIE_NO FROM MOVIE WHERE MOVIE_NAME = '악인전'), 
				THEATER_NO = (SELECT THEATER_NO FROM THEATER WHERE THEATER_NAME = '강남CGV'), 
				MOVIE_THEATER_NO = (SELECT MOVIE_THEATER_NO FROM MOVIE_THEATER WHERE MOVIE_THEATER_NAME = '2관'
										AND THEATER_NO = (SELECT THEATER_NO FROM THEATER WHERE THEATER_NAME = '강남CGV')), 
				MOVIE_START_TIME = '190515 17:21', TIMES = '1회'
				WHERE MOVIE_PLAY_NO = 'MPN1';
...
SELECT TICKETING_NO, MOVIE_PLAY_NO, THEATER_NAME, MOVIE_THEATER_NAME, MOVIE_NAME, MOVIE_START_TIME, SEAT_NO, SEAT_MONEY, ID
	FROM TICKETING JOIN MOVIE_PLAY
		USING(MOVIE_PLAY_NO)
			JOIN THEATER
				USING(THEATER_NO)
					JOIN MOVIE_THEATER
						USING(MOVIE_THEATER_NO)
							JOIN MOVIE
								USING(MOVIE_NO)
		WHERE ID = 'wlstnr7833' AND TICKETING_NO = 'TN82';



DELETE FROM MOVIEMEMBER WHERE MOVIEMEMBER = 'A' CASCADE CONSTRAINT ;
...
INSERT INTO MOVIE_PLAY
			(MOVIE_PLAY_NO, MOVIE_NO, THEATER_NO, MOVIE_THEATER_NO, MOVIE_START_TIME, TIMES)
				VALUES('MPN'||MOVIE_PLAY_NO_SEQ.NEXTVAL, (SELECT MOVIE_NO FROM MOVIE WHERE MOVIE_NAME = '기생충'), (SELECT THEATER_NO FROM THEATER WHERE THEATER_NAME = '강남CGV'), (SELECT MOVIE_THEATER_NO FROM MOVIE_THEATER WHERE MOVIE_THEATER_NAME = '4관' AND THEATER_NO = (SELECT THEATER_NO FROM THEATER WHERE THEATER_NAME = '강남CGV')), '20190605 18:27' ,'4회');


DELETE FROM MOVIE.MOVIE_PLAY
	WHERE MOVIE_PLAY_NO = 'MPN19';

UPDATE TICKETING SET MOVIE_PLAY_NO = NULL 
		WHERE MOVIE_PLAY_NO = 'MPN20';
